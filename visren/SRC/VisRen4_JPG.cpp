...
static int FileGetC(HANDLE infile)
{
  unsigned char read;
  DWORD Readed;
...
#define NUM_FORMATS 12
  ...
  for (int de=0; de<NumDirEntries; de++)
  {
    ...
    {
      case TAG_MAKE:
        lstrcpyn(ImageInfo.CameraMake, (LPCSTR)ValuePtr, 32);
        break;
      case TAG_MODEL:
        lstrcpyn(ImageInfo.CameraModel, (LPCSTR)ValuePtr, 40);
        break;
      case TAG_DATETIME_ORIGINAL:
        for (int i=0; i<19; i++)
        {
          if (ValuePtr[i]==':')
          {
            if (i<10) ImageInfo.DateTime[i]='.';
            else ImageInfo.DateTime[i]='-';
          }
          else ImageInfo.DateTime[i]=ValuePtr[i];
        }
...
static bool ReadJpegSections(HANDLE infile)
{
  ...
  for (int SectionsRead=0; SectionsRead<20; SectionsRead++)
  {
    ...
    UCHAR *Data = (UCHAR *)my_malloc(itemlen);
